...
function aggregateStreamToolCalls(
  events: JsonObject[],
  protocol: string
): AuditToolCall[] {
  const states: StreamToolCallState[] = []
  const stateIndexesByKey = new Map<string, number>()
  const fallbackCalls: { call: AuditToolCall; order: number }[] = []

  events.forEach((event, eventPosition) => {
    const fragments = extractStreamToolCallFragments(event, protocol)
    fragments.forEach((fragment, fragmentPosition) => {
      mergeStreamToolCallFragment(
        states,
        stateIndexesByKey,
        fragment,
        eventPosition * 1000 + fragmentPosition
      )
    })

    extractStreamToolCalls(event, protocol).forEach((call, callPosition) => {
      if (
        call.kind === 'result' ||
        isImageGenerationToolCall(call) ||
        fragments.length === 0
      ) {
        fallbackCalls.push({
          call,
          order: eventPosition * 1000 + fragments.length + callPosition,
        })
      }
    })
  })
  ...
  const orderedCalls = [
    ...states
      ...
      .map((state) => ({
        call: streamToolCallStateToAuditToolCall(state),
        order: state.order,
      })),
    ...fallbackCalls,
  ]
    ...
    .map((item) => item.call)
  return compactSseToolCalls(orderedCalls)
}

function extractStreamToolCallFragments(
  event: JsonObject,
  protocol: string
): StreamToolCallFragment[] {
  if (protocol === 'claude_messages') {
    return extractClaudeStreamToolCallFragments(event)
  }
  if (
    protocol === 'openai_responses' ||
    protocol === 'openai_responses_compact'
  ) {
    return extractOpenAIResponsesStreamToolCallFragments(event)
  }
  if (protocol === 'gemini_generate_content') {
    return extractGeminiStreamToolCallFragments(event)
  }
  return extractOpenAIChatStreamToolCallFragments(event)
}

function extractOpenAIChatStreamToolCallFragments(
  event: JsonObject
): StreamToolCallFragment[] {
  return arrayValue(event.choices).flatMap((choice, choicePosition) => {
    if (!isRecord(choice)) return []
    const choiceIndex = firstString(choice.index) || String(choicePosition)
    const fragments: StreamToolCallFragment[] = []

    for (const [source, inputMode] of [
      [choice.delta, 'delta'],
      [choice.message, 'snapshot'],
    ] as const) {
      if (!isRecord(source)) continue

      arrayValue(source.tool_calls).forEach((value, toolPosition) => {
        if (!isRecord(value)) return
        const fn = isRecord(value.function) ? value.function : value
        const toolIndex = firstString(value.index) || String(toolPosition)
        const id = extractToolIdentity(value)
        fragments.push({
          key: `openai:choice:${choiceIndex}:tool:${toolIndex}`,
          aliases: id ? [`tool-id:${id}`] : [],
          id,
          name: firstString(fn.name, value.name) || undefined,
          type: stringValue(value.type) || 'tool_call',
          inputValue: firstDefined(fn.arguments, fn.args, fn.input),
          inputMode,
          raw: event,
        })
      })

      if (isRecord(source.function_call)) {
        fragments.push({
          key: `openai:choice:${choiceIndex}:legacy-function`,
          aliases: [],
          name: firstString(source.function_call.name) || undefined,
          type: 'function_call',
          inputValue: firstDefined(
            source.function_call.arguments,
            source.function_call.args,
            source.function_call.input
          ),
          inputMode,
          raw: event,
        })
...
function extractClaudeStreamToolCallFragments(
  event: JsonObject
): StreamToolCallFragment[] {
  const eventType = firstString(event.type, event.event).toLowerCase()
  const blockIndex = firstString(event.index)
  if (!blockIndex) return []
  const key = `claude:content-block:${blockIndex}`

  if (
    eventType === 'content_block_start' &&
    isRecord(event.content_block) &&
    stringValue(event.content_block.type).toLowerCase() === 'tool_use'
  ) {
    const id = extractToolIdentity(event.content_block)
    return [
      {
        key,
        aliases: id ? [`tool-id:${id}`] : [],
        id,
        name: firstString(event.content_block.name) || undefined,
        type: 'tool_use',
  ...
  if (
    ...
    stringValue(event.delta.type).toLowerCase() === 'input_json_delta'
  ) {
    return [
      {
        key,
...
function extractOpenAIResponsesStreamToolCallFragments(
  event: JsonObject
): StreamToolCallFragment[] {
  const fragments: StreamToolCallFragment[] = []
  const eventType = firstString(event.type, event.event).toLowerCase()
  ...
  if (
    ...
    const ids = streamToolIdentityValues(event)
    if (ids.length > 0) {
      fragments.push({
        key: `responses:${ids[0]}`,
        aliases: ids.map((id) => `responses:${id}`),
        id: preferredStreamToolIdentity(ids),
        name: firstString(event.name, event.function_name) || undefined,
        type: 'function_call',
        inputValue: firstDefined(event.arguments, event.delta, event.input),
        inputMode: eventType.endsWith('.delta') ? 'delta' : 'snapshot',
        raw: event,
      })
    }
  }
  ...
  if (isRecord(event.item)) {
    const fragment = openAIResponsesItemFragment(event.item, event)
    if (fragment) fragments.push(fragment)
  }

  if (isRecord(event.response)) {
    for (const item of arrayValue(event.response.output)) {
      if (!isRecord(item)) continue
      const fragment = openAIResponsesItemFragment(item, event)
      if (fragment) fragments.push(fragment)
    }
  }
...
function openAIResponsesItemFragment(
  item: JsonObject,
  rawEvent: JsonObject
): StreamToolCallFragment | undefined {
  if (!isOpenAIResponseToolOutputType(stringValue(item.type))) return undefined
  if (TOOL_RESULT_TYPES.has(stringValue(item.type).toLowerCase())) {
    return undefined
  }

  const ids = streamToolIdentityValues(item)
  if (ids.length === 0) return undefined
  return {
    key: `responses:${ids[0]}`,
    aliases: ids.map((id) => `responses:${id}`),
    id: preferredStreamToolIdentity(ids),
    name: firstString(item.name, item.function_name) || undefined,
    type: stringValue(item.type) || 'function_call',
    inputValue: firstDefined(item.arguments, item.input, item.args),
    inputMode: 'snapshot',
    raw: rawEvent,
  }
}

function extractGeminiStreamToolCallFragments(
  event: JsonObject
): StreamToolCallFragment[] {
  return arrayValue(event.candidates).flatMap(
    (candidate, candidatePosition) => {
      if (!isRecord(candidate) || !isRecord(candidate.content)) return []
      const candidateIndex =
        firstString(candidate.index) || String(candidatePosition)
      return arrayValue(candidate.content.parts).flatMap(
        (part, partPosition) => {
          if (!isRecord(part) || !isRecord(part.functionCall)) return []
          const id = extractToolIdentity(part.functionCall)
          return [
            {
              key: `gemini:candidate:${candidateIndex}:function:${partPosition}`,
              aliases: id ? [`tool-id:${id}`] : [],
              id,
              name: firstString(part.functionCall.name) || undefined,
              type: 'function_call',
              inputValue: firstDefined(
                part.functionCall.args,
                part.functionCall.arguments,
                part.functionCall.input
              ),
              inputMode: 'snapshot',
              raw: event,
            },
...
function mergeStreamToolCallFragment(
  states: StreamToolCallState[],
  stateIndexesByKey: Map<string, number>,
  fragment: StreamToolCallFragment,
  order: number
): void {
  const keys = [fragment.key, ...fragment.aliases]
  const existingIndexes = [
    ...new Set(
      keys
        .map((key) => stateIndexesByKey.get(key))
  ...
  if (existingIndex === undefined) {
    const state: StreamToolCallState = {
      ...
      order,
      id: fragment.id,
      name: fragment.name || 'tool_call',
      type: fragment.type,
      inputValue: mergeStreamToolInput(
        undefined,
        fragment.inputValue,
        fragment.inputMode
      ),
      rawEvents: [fragment.raw],
    }
    const nextIndex = states.length
    states.push(state)
    for (const key of keys) stateIndexesByKey.set(key, nextIndex)
    return
  }

  const state = states.at(existingIndex)
  if (!state) return
  for (const mergedIndex of existingIndexes.slice(1)) {
    const mergedState = states.at(mergedIndex)
    if (!mergedState?.active) continue
    mergeStreamToolCallStates(state, mergedState)
    mergedState.active = false
    for (const [key, index] of stateIndexesByKey) {
      if (index === mergedIndex) stateIndexesByKey.set(key, existingIndex)
    }
  }
  for (const key of keys) {
    stateIndexesByKey.set(key, existingIndex)
  }
  if (fragment.id) {
    const ids = [state.id, fragment.id].filter((id): id is string =>
      Boolean(id)
    )
    state.id = preferredStreamToolIdentity(ids)
  }
  if (fragment.name && isFallbackToolName(state.name, state.type)) {
    state.name = fragment.name
  }
  if (fragment.type) state.type = fragment.type
  state.inputValue = mergeStreamToolInput(
    state.inputValue,
    fragment.inputValue,
    fragment.inputMode
  )
  if (state.rawEvents.at(-1) !== fragment.raw) {
    state.rawEvents.push(fragment.raw)
  }
}

function mergeStreamToolCallStates(
  target: StreamToolCallState,
  source: StreamToolCallState
): void {
  const ids = [target.id, source.id].filter((id): id is string => Boolean(id))
  target.id = preferredStreamToolIdentity(ids)
  target.order = Math.min(target.order, source.order)
  if (isFallbackToolName(target.name, target.type)) {
    target.name = source.name
  }
  if (!target.type) target.type = source.type
  target.inputValue = mergeToolInputValue(target.inputValue, source.inputValue)
  for (const rawEvent of source.rawEvents) {
    if (target.rawEvents.at(-1) !== rawEvent) {
      target.rawEvents.push(rawEvent)
    }
  }
}

function mergeStreamToolInput(
  existing: unknown,
  incoming: unknown,
  mode: 'delta' | 'snapshot'
): unknown {
  ...
  if (mode === 'delta') {
    const delta = stringValue(incoming)
  ...
  if (typeof existing === 'string' && typeof incoming === 'string') {
    if (incoming === existing || incoming.includes(existing)) return incoming
    if (existing.includes(incoming)) return existing
  }
  return incoming
}

function streamToolCallStateToAuditToolCall(
  state: StreamToolCallState
): AuditToolCall {
  const inputValue = normalizeNestedJson(state.inputValue)
  return {
    kind: 'call',
    id: state.id,
    name: state.name,
    type: state.type,
    input: formatUnknown(inputValue),
    inputValue,
    inputFields: toToolFieldRows(inputValue),
    summary: summarizeToolArguments(inputValue),
    raw: formatUnknown(
      state.rawEvents.length === 1 ? state.rawEvents[0] : state.rawEvents
    ),
  }
}

function streamToolIdentityValues(record: JsonObject): string[] {
...
function preferredStreamToolIdentity(ids: string[]): string | undefined {
...
function extractOpenAIResponseFunctionArgumentEvents(
  ...
  const id = firstString(event.item_id, event.call_id, event.id) || undefined
  if (!id) return []
...
function preferredToolLifecycleId(
  ...
  const ids = [better.id, incoming.id, existing.id].filter((id): id is string =>
    Boolean(id)
  )
  return ids.find((id) => id.startsWith('call_')) || ids[0]
}
